...
- Must-be（基本型）
  - 统一认证与访问控制、Offer生命周期看板与KPI/ROSC、Adscenter预检与批量的预览/审计/回滚、事件驱动与读模型、原子计费、基础风险识别与通知。
- Performance（绩效型）
  - 10秒评估分阶段SLO与降级、批量成功率与吞吐、Browser-Exec并发与稳定性、全局多账户聚合性能、预警召回/精准度与节流、数据同步时效、国际化与SEO。
- Attractive（魅力型）
  - 机会雷达与相似推荐、一键诊断与一键修复、A/B测试统计显著性与推荐、变更计划可视化、换链接时间轴。

默认策略与反向需求防护
- 自动化默认“建议+确认”，仅在用户显式开启时才自动处置。
- 预警须实施节流、聚类合并与置信度门限，降低通知噪音。
- 换链接/批量变更需配置频控与回退策略，保障用户“掌控感”。
...
7. WHEN Offer连续5天出现0曝光0点击 THEN 系统 SHALL 生成“转入衰退期”建议并通知用户；若用户在设置中显式开启自动化，则自动更新状态
...
9. WHEN 用户在设置中开启或关闭“自动状态转换” THEN 系统 SHALL 立即生效，并在状态自动更新时写入审计快照与通知
...
**规则能力：** 低曝低点/无点/落地不可用/数据缺失；动作：提示/建任务/触发工作流/自动处置（默认仅提示/建任务，自动处置需显式开启）
...
8. WHEN 用户查看趋势图 THEN 系统 SHALL 支持7天、30天、90天等不同时间范围的趋势分析
...
13. WHEN A/B测试运行 THEN 系统 SHALL 实时显示测试进度和统计显著性，并在达到置信度时推荐获胜版本
14. WHEN 配置换链接 THEN 系统 SHALL 提供最小/最大频控与每Offer/每账户速率限制，异常时自动回退上次稳定配置
...
11. WHEN 检测到系统运行异常 THEN 系统 SHALL 立即暂停相关任务并通知用户和管理员
12. WHEN 推送预警 THEN 系统 SHALL 对通知进行节流、聚类合并与置信度门限控制，避免高频噪音
13. WHEN 用户在设置中配置通知参数 THEN 系统 SHALL 支持启停、最小置信度、每分钟阈值、聚类窗口和通知通道切换
...
6. WHEN Max/Elite套餐用户使用批量功能 THEN 系统 SHALL 支持批量评估、批量仿真等高级操作### 需求12：后台管理系统
