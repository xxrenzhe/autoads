...
- 事件系统与UI缓存（本次新增）
  ...
  - offer：迁移发布到 pkg/events（OfferCreated）；移除服务内自投影，统一由 notifications 投影到 Offer 读模型
  - batchopen：创建任务发布 BatchOpsTaskQueued；同时写入 Firestore UI 缓存 users/{uid}/batchopen/tasks/{taskId}
  - adscenter：Preflight 结果写入 Firestore UI 缓存 users/{uid}/adscenter/preflight/{accountId}
  - billing：查询订阅/Token 余额时写入 Firestore UI 缓存 users/{uid}/billing/summary（merge）
