...
export class TokenRuleService {
  ...
  private async initializeCache(): Promise<void> {
    try {
      ...
      const defaultRules = {
        ...
        'changelink-default': parseInt(process.env.CHANGELINK_TOKEN_COST || '1'),
      };

      // Set default rules in cache
      Object.entries(defaultRules).forEach(([key, cost]) => {
  ...
  private async updateSystemRule(
    ...
    const envVarMap: { [key: string]: string } = {
      ...
      'changelink-default': 'CHANGELINK_TOKEN_COST',
    };
  ...
  public async getAllRules(): Promise<Array<{
    ...
    const envRules = [
      ...
      { id: 'changelink-default', feature: 'changelink', method: 'default' },
    ];
