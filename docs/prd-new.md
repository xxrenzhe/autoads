...
| v25.0 | 2025-01-10 | 全面修正文档不一致：统一功能名称（使用"批量访问"而非"真实点击"，使用"链接管理"而非"自动化广告"）；修正并发性能提升描述（4900%而非500%）；确保整个文档术语一致性 | 产品团队 |
