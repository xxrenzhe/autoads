/**
 * 执行编排器 - 核心自动化流程管理
 * 负责协调AdsPower、URL提取、Google Ads更新的完整流程
 */

import { ConfigurationManager } from './ConfigurationManager';
import { TrackingConfiguration } from '../types';
import { ExecutionReport } from './NotificationService';
import { AdsPowerApiClient } from './AdsPowerApiClient';
import { UrlExtractionService, UrlExtractionResult } from './UrlExtractionService';
...
// Storage key mapping for compatibility with new LocalStorageService
const STORAGE_KEYS = {
...
const logger = createClientLogger('ExecutionOrchestrator');
...
export interface LinkProcessingResult {
  originalUrl: string;
  extractionResults: UrlExtractionResult[];
...
export class ExecutionOrchestrator {
  ...
  constructor(
    ...
    storageService: typeof localStorageService,
  ...
  async startExecution(configurationId: string, userId: string): Promise<string> {
    ...
    try {
      ...
      // 3. 在本地存储中创建执行记录
      await this.storageService.create(STORAGE_KEYS.EXECUTIONS, { configuration_id: configurationId,
        user_id: userId,
        total_items: context.metadata.totalLinks * config.repeatCount
      });
      // 4. 异步执行主流程
  ...
  private async processLink(
    ...
    try {
      ...
      for (let execution = 1; execution <= (config.repeatCount || 1); execution++) {
        ...
        // 提取URL
        const extractionResult = await this.urlExtractionService.extractFinalUrl({ environmentId: config.environmentId!,
          originalUrl,
          timeout: 60000,
          headless: true,
        ...
        if (extractionResult.success) { // 2. 更新对应的Google Ads
            ...
            for (const adMapping of adsToUpdate) {
              try {
                ...
                if (account) {
                  ...
                  result.googleAdsUpdates.push({ adId: adMapping.adId,
              ...
              } catch (error) { result.googleAdsUpdates.push({
                  adId: adMapping.adId,
   ...
   * 生成执行报告
   */
  private generateExecutionReport(context: ExecutionContext, linkResults: LinkProcessingResult[]): ExecutionReport {
  ...
  async getExecutionStatus(executionId: string): Promise<ExecutionContext | null> {
    ...
    // 如果内存中没有，尝试从本地存储获取
    const dbExecution = await this.storageService.getById(STORAGE_KEYS.EXECUTIONS, executionId);
    if (dbExecution) {
      return {
        id: dbExecution.id,
        configurationId: dbExecution.data.configuration_id,
        userId: dbExecution.data.user_id,
        status: dbExecution.data.status as any,
        progress: dbExecution.data.progress,
        totalSteps: 0,
        currentStep: '已完成',
        startTime: dbExecution.data.started_at,
        endTime: dbExecution.data.completed_at,
        results: [],
        errors: [],
        metadata: {
          totalLinks: dbExecution.data.total_items,
          processedLinks: dbExecution.data.processed_items,
          successfulUpdates: 0,
          failedUpdates: 0
        }
      };
  ...
  private async updateExecutionInDB(context: ExecutionContext): Promise<void> {
    try {
      await this.storageService.update(STORAGE_KEYS.EXECUTIONS, context.id, {
