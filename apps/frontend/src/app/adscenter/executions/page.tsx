...
export default function ExecutionsPage() {
  ...
  useEffect(() => {
    ...
    try {
      es = new EventSource('/api/adscenter/executions/events');
      es.onmessage = (ev) => {
        try {
          ...
          if (msg.type === 'execution_update') {
            setActiveExecutions((prev) => {
              const next = [...prev];
              const idx = next.findIndex(x => x.id === msg.id);
              if (idx !== -1) {
                next[idx] = {
                  ...next[idx],
                  status: msg.status || next[idx].status,
                  progress: typeof msg.progress === 'number' ? msg.progress : next[idx].progress,
                  metadata: {
                    ...next[idx].metadata,
                    totalLinks: typeof msg.totalItems === 'number' ? msg.totalItems : next[idx].metadata.totalLinks,
                    processedLinks: typeof msg.processedItems === 'number' ? msg.processedItems : next[idx].metadata.processedLinks,
                  },
                  endTime: msg.completedAt || next[idx].endTime,
                };
              } else if (msg.status === 'created' && msg.id) {
                next.unshift({
                  id: msg.id,
                  configurationId: msg.configurationId || 'unknown',
                  userId: 'me',
                  status: 'pending',
                  progress: 0,
                  totalSteps: 1,
                  ...
                  results: [],
                  errors: [],
                  metadata: { totalLinks: 0, processedLinks: 0, successfulUpdates: 0, failedUpdates: 0 }
                });
              }
              return next;
            });
            ...
            if (['completed', 'failed', 'cancelled'].includes(msg.status)) {
              setActiveExecutions((prev) => prev.filter(x => x.id !== msg.id));
              setHistoryExecutions((prev) => [{
                id: msg.id,
                configurationId: msg.configurationId || 'unknown',
                userId: 'me',
                status: msg.status,
                ...
                startTime: new Date().toISOString(),
                endTime: msg.completedAt || new Date().toISOString(),
                results: [], errors: [],
                metadata: { totalLinks: msg.totalItems || 0, processedLinks: msg.processedItems || 0, successfulUpdates: 0, failedUpdates: 0 }
              }, ...prev]);
            }
          }
        } catch {}
  ...
  // 手动标记进度（演示/检修用）
  const handleBumpProgress = async (execution: ExecutionContext, delta: number = 10) => {
    try {
      const next = Math.max(0, Math.min(100, execution.progress + delta));
      const processed = Math.max(0, Math.min(execution.metadata.totalLinks, execution.metadata.processedLinks + Math.ceil((delta/100) * Math.max(1, execution.metadata.totalLinks))));
      await http.patch(`/adscenter/executions/${execution.id}`, {
        progress: next,
        processedItems: processed,
        totalItems: execution.metadata.totalLinks,
  ...
  const handleMarkCompleted = async (execution: ExecutionContext) => {
    try {
      await http.patch(`/adscenter/executions/${execution.id}`, {
        status: 'completed',
        progress: 100,
        processedItems: execution.metadata.totalLinks,
        totalItems: execution.metadata.totalLinks,
        startedAt: execution.startTime,
    ...
    <Card className="hover:shadow-lg transition-shadow">
      ...
      <CardContent>
        <div className="space-y-3">
          ...
          {isActive && execution.status === 'running' && (
            <>
              <Button size="sm" variant="outline" onClick={() => handleBumpProgress(execution, 10)}>
                <TrendingUp className="h-4 w-4 mr-1" /> 进度+10%
              </Button>
              <Button size="sm" onClick={() => handleMarkCompleted(execution)}>
