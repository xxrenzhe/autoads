...
export async function PUT(req: Request, ctx: { params: { id: string }}) {
  ...
  const upstream = `${CONSOLE_SERVICE_URL}/api/v1/console/users/${id}/role`;
  const res = await fetch(upstream, { method:'PUT', headers:{ Authorization:`Bearer ${token}`, 'Content-Type':'application/json' }, body });
