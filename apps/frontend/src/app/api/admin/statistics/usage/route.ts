...
/**
 * Get usage statistics for admin dashboard (optimized)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Use optimized cached service
    const [coreMetrics, usageStats] = await Promise.all([
      statisticsCacheService.getCoreMetrics('30'),
      statisticsCacheService.getUsageStatistics(filters)
    ]);
