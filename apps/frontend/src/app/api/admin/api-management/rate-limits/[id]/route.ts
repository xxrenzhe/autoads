...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    rules[idx] = { ...rules[idx], ...body, updatedAt: new Date().toISOString() }
    ...
    return NextResponse.json({ success: true, data: rules[idx] })
  } catch (err) {
    console.error('Update rate limit rule error:', err)
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    let rules: any[] = []
    try { rules = cfg?.value ? JSON.parse(cfg.value) : [] } catch {}
    const next = rules.filter((r: any) => r.id !== id)
    if (next.length === rules.length) return NextResponse.json({ error: 'Rule not found' }, { status: 404 })
    await prisma.systemConfig.upsert({
      where: { key: CONFIG_KEY },
      update: { value: JSON.stringify(next), updatedBy: session.user.id, updatedAt: new Date() },
      create: { key: CONFIG_KEY, value: JSON.stringify(next), category: 'api-management', description: 'API Management: rate limit rules', isSecret: false, createdBy: session.user.id, updatedBy: session.user.id }
    })
    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('Delete rate limit rule error:', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
