...
export class ConfigurationValidator {
  ...
  async validateEnvironment(environmentId: string): Promise<ValidationResult> {
    try {
      ...
      // 检查环境是否存在
      const environment = data.data?.list?.find((env: Record<string, unknown>) => (env as { user_id: string }).user_id === environmentId);
  ...
  async getEnvironmentList(): Promise<EnvironmentInfo[]> {
    try {
      ...
      return data.data?.list?.map((env: Record<string, unknown>) => ({
        id: env.user_id,
        name: env.name,
