...
const BulkRoleButton: React.FC<{ selectedIds?: string[] }> = ({ selectedIds = [] }) => {
  ...
  const handleRoleChange = async () => {
    ...
    try {
      ...
      await Promise.all(
        selectedIds.map((userId: string: any) =>
    ...
    <>
      ...
      >
        <NumberInput
          ...
          label="充值数量"
          value={amount}
          onChange={((e: any: any): any) => setAmount(e.target.value)}
          min={1}
          max={10000}
        />
      </Confirm>
        ...
        selectedIds.map((userId: string: any) =>
          fetch(`/api/admin/users/${userId}/subscription`, {
            ...
            body: JSON.stringify({ 
              planId: selectedPlan,
              duration,
              notes: `批量分配: ${plans.find((p: any) => p.id === selectedPlan)?.name}`
            }),
          })
        )
      );
      
      const planName = plans.find((p: any) => p.id === selectedPlan)?.name || 'selected plan';
    ...
    <>
      ...
      >
        ...
        <NumberInput
          ...
          label="持续时间（月）"
          value={duration}
          onChange={((e: any: any): any) => setDuration(parseInt(e.target.value) || 1)}
          min={1}
          max={12}
          fullWidth
          style={{ marginTop: 16 }}
