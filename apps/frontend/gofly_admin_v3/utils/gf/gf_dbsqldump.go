...
func DBDump(opts ...DumpOption) error {
	dsn := GetDB_dsn()
	// 打印开始
	start := time.Now()
	...
	// 1. 获取数据库
	dbName, err := GetDBNameFromDNS(dsn)
	if err != nil {
		log.Printf("[error] %v \n", err)
		return err
	}
...
func GetDB_dsn() string {
	if String(dbConf_arr["type"]) == "mysql" || String(dbConf_arr["type"]) == "mariadb" || String(dbConf_arr["type"]) == "tidb" {
		return fmt.Sprintf("%v:%v@tcp(%v:%v)/%v?charset=utf8&parseTime=True&loc=Local&timeout=1000ms&sql_mode=%s&multiStatements=true", dbConf_arr["username"], dbConf_arr["password"], dbConf_arr["hostname"], dbConf_arr["hostport"], dbConf_arr["dbname"], dbConf_arr["sqlmode"])
