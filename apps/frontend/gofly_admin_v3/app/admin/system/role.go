...
func (api *Role) GetList(c *gf.GinCtx) {
	...
	whereMap := gmap.New()
	if !gf.IntInInterfaceArray(1, all_role_id) {
	...
	roleList, _ := gf.Model("admin_auth_role").Where(whereMap).Order("weigh asc").Select()
	//获取最大一级的pid
	if len(roleList) > 0 {
		max_role_id, _ := gf.Model("admin_auth_role").Where(whereMap).Order("id asc").Value("pid")
		roleList = gf.GetTreeArray(roleList, gf.Int64(max_role_id), "")
	}
	max_pid, _ := gf.Model("admin_auth_role").Where("id", user_role_ids).Order("id asc").Value("pid")
	gf.Success().SetMsg("获取拥有角色列表").SetData(gf.Map{"list": roleList, "max_pid": max_pid}).Regin(c)
}
...
func (api *Role) GetParent(c *gf.GinCtx) {
	...
	// role_ids := gf.GetAllChilIds("admin_auth_role", user_role_ids) //批量获取子节点id
	// all_role_id := gf.MergeArr(user_role_ids, role_ids)
	// menuList, _ := gf.Model("admin_auth_role").WhereIn("id", all_role_id).Fields("id,pid,name").Order("weigh asc").Select()
	...
	//获取自己权限组-显示自己所在的权限组
	my_role_account_id, _ := gf.Model("admin_auth_role").WhereIn("id", user_role_ids).Value("account_id")
	account_id = append(account_id, my_role_account_id)
	whereMap.Set("uid IN(?)", account_id)
	if gf.Int64(id) != 0 {
		whereMap.Set("id !=?", id)
	}
	roleList, _ := gf.Model("admin_auth_role").Where(whereMap).Order("weigh asc").Select()
	...
	gf.Success().SetMsg("表单获取选择角色组父级数据").SetData(roleList).Regin(c)
}

// 表单获取菜单
func (api *Role) GetMenuList(c *gf.GinCtx) {
	pid := c.DefaultQuery("pid", "0")
	MDB := gf.Model("admin_auth_rule").Where("status", 0).WhereIn("type", []interface{}{0, 1})
	var rule_ids []interface{}
	if pid == "0" { //获取本账号所拥有的权限
		userID := c.GetInt64("userID") //当前用户ID
		role_id, _ := gf.Model("admin_auth_role_access").Where("uid", userID).Array("role_id")
		menu_id, _ := gf.Model("admin_auth_role").WhereIn("id", role_id).Array("rules")
		super_role, _ := gf.Model("admin_auth_role").WhereIn("id", role_id).Where("rules", "*").Value("id")
		if super_role == nil { //不是超级权限-过滤菜单权限
			getmenus := ArraymoreMerge(menu_id)
			MDB = MDB.WhereIn("id", getmenus)
			rule_ids = getmenus
		}
	} else {
		//获取用户权限
		menu_id_str, _ := gf.Model("admin_auth_role").Where("id", pid).Value("rules")
	...
	for _, val := range menuList {
		...
		btn_rules, _ := gf.Model("admin_auth_rule").Where("status", 0).Where("type", 2).Where("pid", val["id"]).Where(whereMap).Fields("id,pid,title,locale").Order("weigh asc").Select()
		if btn_rules != nil && len(btn_rules) > 0 {
			...
			//一级菜单获取子级菜单按钮
			sub_rule_ids, _ := gf.Model("admin_auth_rule").Where("pid", val["id"]).Where("status", 0).Where("type !=", 2).Array("id")
			btn_rule_ids, _ := gf.Model("admin_auth_rule").Where("status", 0).Where("type", 2).WhereIn("pid", sub_rule_ids).Array("id")
	...
	if rule_ids == nil {
		btn_idsdata, _ := gf.Model("admin_auth_rule").Where("status", 0).Where("type", 2).Array("id")
		gf.Success().SetMsg("获取菜单数据1").SetData(gf.Map{"list": menuList, "btn_rule_ids": btn_idsdata}).Regin(c)
	} else {
		btn_idsdata, _ := gf.Model("admin_auth_rule").Where("status", 0).Where("type", 2).WhereIn("id", rule_ids).Array("id")
...
func (api *Role) Save(c *gf.GinCtx) {
	...
	if f_id == 0 {
		delete(param, "id")
		addId, err := gf.Model("admin_auth_role").Data(param).InsertAndGetId()
		if err != nil {
			gf.Failed().SetMsg("添加失败").SetData(err).Regin(c)
		} else {
			if addId != 0 {
				gf.Model("admin_auth_role").Data(map[string]interface{}{"weigh": addId}).Where("id", addId).Update()
			}
		...
		delete(param, "spacer")
		res, err := gf.Model("admin_auth_role").Data(param).Where("id", f_id).Update()
		if err != nil {
...
func (api *Role) UpStatus(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	res2, err := gf.Model("admin_auth_role").Where("id", param["id"]).Data(map[string]interface{}{"status": param["status"]}).Update()
	if err != nil {
		gf.Failed().SetMsg("更新失败！").SetData(err).Regin(c)
	} else {
		msg := "更新成功！"
...
func (api *Role) Del(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	res2, err := gf.Model("admin_auth_role").WhereIn("id", param["ids"]).Delete()
	if err != nil {
