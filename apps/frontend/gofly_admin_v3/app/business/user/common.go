...
func GetMenuArray(pdata gf.OrmResult, parent_id int64, roles []interface{}) []map[string]interface{} {
	...
	for _, v := range pdata {
		if v["pid"].Int64() == parent_id {
			...
			//11.按钮权限
			if len(roles) == 0 { //超级权限
				permission, _ := gf.Model("business_auth_rule").Where("status", 0).Where("type", 2).Where("pid", v["id"]).WhereNotNull("permission").Array("permission")
				...
				permission, _ := gf.Model("business_auth_rule").Where("status", 0).Where("type", 2).Where("pid", v["id"]).WhereIn("id", roles).WhereNotNull("permission").Array("permission")
				if permission != nil && len(permission) > 0 {
					meta["btnroles"] = permission
				} else {
					hasepermission, _ := gf.Model("business_auth_rule").Where("status", 0).Where("type", 2).Where("pid", v["id"]).WhereNotNull("permission").Array("permission")
