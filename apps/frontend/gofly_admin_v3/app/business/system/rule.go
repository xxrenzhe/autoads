...
func (api *Rule) GetParent(c *gf.GinCtx) {
	...
	menuList, err := gf.Model("business_auth_rule").WhereIn("type", []interface{}{0, 1}).Where("id !=", id).Fields("id,pid,title,locale,routepath").Order("weigh asc").Select()
	if err != nil {
...
func (api *Rule) GetRoutes(c *gf.GinCtx) {
	...
	for _, val := range list {
		...
		if len(item) == 2 && !strings.HasPrefix(item[1], "/admin/") && !strings.Contains(item[1], "/developer/") && !strings.HasPrefix(item[1], "/common/") {
...
func (api *Rule) UpStatus(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	res, err := gf.Model("business_auth_rule").Where("id", param["id"]).Data(map[string]interface{}{"status": param["status"]}).Update()
	if err != nil {
