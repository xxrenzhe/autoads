...
type ConfigNode struct {
	// Hostname specifies the server address, can be either IP address or domain name
	// Example: "127.0.0.1", "localhost"
	Hostname string `json:"hostname"`

	// Hostport specifies the server port number
	// Default is typically "3306" for MySQL
	Hostport string `json:"hostport"`

	// username specifies the authentication username for database connection
	Username string `json:"username"`

	// Password specifies the authentication password for database connection
	Password string `json:"password"`

	// Name specifies the default database name to be used
	Dbname string `json:"dbname"`

	//  Set the database sql_mode, when the database type mariadb/mysql can be set
...
func (c *Core) GetConfig() *ConfigNode {
	...
	if configNode != nil {
		...
		if c.config.Dbname == configNode.Dbname {
			return configNode
		}
	}
	return c.config
